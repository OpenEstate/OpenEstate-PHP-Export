...
class ImmoToolFilter_region extends ImmoToolFilter {
  ...
  function filter($object, &$items) {
    $region = (isset($object['adress']['region'])) ? $object['adress']['region'] : null;
    if (!is_string($region))
      return;
    $region = trim($region);
    if (strlen($region) == 0)
      return;
    if (!isset($items[$region]) || !is_array($items[$region]))
      $items[$region] = array();
    $items[$region][] = $object['id'];
  }
  ...
  function getTitle(&$translations, $lang) {
    $title = (isset($translations['labels']['estate.region'])) ? $translations['labels']['estate.region'] : null;
