 ...
 * @package Net
 * @version $Id: php4.php 284681 2009-07-24 04:24:27Z clockwerx $
 */
class Net_IDNA_php4 {
  ...
  function _nameprep($input) {
    ...
    foreach ($input as $v) {
      ...
      if (in_array($v, $this->_np_map_nothing))
        continue;
    ...
    while ($swap) {
      ...
      for ($i = 0; $i < $size - 1; ++$i) {
        ...
        if ($next != 0 && $last > $next) {
          ...
          for ($j = $i + 1; $j > 0; --$j) {
            if ($this->_get_combining_class($input[$j - 1]) <= $next)
              break;
            $t = $input[$j];
            $input[$j] = $input[$j - 1];
            $input[$j - 1] = $t;
  ...
  function _combine($input) {
    ...
    if (1 != $inp_len) {
      ...
      if (count($hangul) != $inp_len)
        return $hangul; // This place is probably wrong
    }
